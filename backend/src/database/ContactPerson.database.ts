...
const COL_FK_PEOPLE = "user_document";
const COL_FK_CONTACTS = "contact_document";

export default {
  createTable: (): string => `CREATE TABLE IF NOT EXISTS ${TABLE_NAME} (${COL_USER_DOCUMENT} TEXT PRIMARY KEY NOT NULL, 
    ${COL_CONTACT_DOCUMENT} TEXT PRIMARY KEY NOT NULL),
    CONSTRAINT ${FOREIGN_KEY_PEOPLE} FOREIGN KEY (${COL_USER_DOCUMENT}), REFERENCES ${FOREIGN_TABLE_PEOPLE}(${COL_FK_PEOPLE}),
    CONSTRAINT ${FOREIGN_KEY_CONTACTS} FOREIGN KEY (${COL_USER_DOCUMENT}), REFERENCES ${FOREIGN_TABLE_CONTACTS}(${COL_FK_CONTACTS}))`,
