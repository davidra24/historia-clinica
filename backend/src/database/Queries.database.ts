...
const COL_FK_PEOPLE = "professional_document";
const COL_FK_ATTENTIONCENTERS = "id_center";

export default {
  createTable: (): string => `CREATE TABLE IF NOT EXISTS ${TABLE_NAME} (${COL_ID} UUID PRIMARY KEY NOT NULL DEFAULT uuid_generate_v4(), 
    ${COL_DATE} DATE NOT NULL, ${COL_USER_DOCUMENT} TEXT NOT NULL, ${COL_ID_CENTER} NUMBER NOT NULL, 
    ...
    CONSTRAINT ${FOREIGN_KEY_ATTENTIONCENTERS} FOREIGN KEY (${COL_ID_CENTER}), REFERENCES ${FOREIGN_TABLE_ATTENTIONCENTERS}(${COL_FK_ATTENTIONCENTERS}))`,
